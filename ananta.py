...
import pywhatkit                   #for multiple features
...
def Process_audio():
    ...
    if __name__ == '__main__':
        while run == 1:
            ...
            if "hello" in statement or "new" in statement:
                name()
                wishMe()
            ...
            if 'screenshot' in statement:
                speak('Please tell me what to name this file')
                nameanswer = get_audio().lower()
                speak('hold on for a few seconds, im taking the screenshot')
                time.sleep(3)
                img = pyautogui.screenshot()
                img.save(f"{nameanswer}.png")
                speak("i am done taking the screenshot, it is in the main folder")
